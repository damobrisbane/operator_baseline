...
_f_grpc_csvJson() {
  ...
  jq -c '.csvJson|fromjson' <<<$(grpcurl -plaintext -d "{\"pkgName\":\"${_OP}\",\"channelName\":\"${_CH}\"}" $_GRPC_URL api.Registry/GetBundleForChannel)
}
...
_f_grpc_get_packages() {
        ...
        for _CH in ${_L_CHNLS[@]}; do
          ...
          if [[ -z ${_A_IN_SET[$_CH]} ]]; then
            ...
            if [[ ( $_CH == $_STOCK_CH || $_CH == $_DEF_CH_NAME ) && -z ${_A_IN_SET[$_CH]} ]]; then
                ...
                local _CSVJSON=$(_f_grpc_csvJson $_GRPC_URL $_PKG $_CH)

                local _RELATED_IMAGES=$(jq '.spec|[.relatedImages[].image]|join("#")' <<<$_CSVJSON)

                local _VERSION=$(jq -r .version <<<$_CSVJSON)
  ...
  _J_PKGS_CUT_1999+=$_J_PKG

  done

  #jq -sc . <<<$_J_PKGS_CUT_1999
}
